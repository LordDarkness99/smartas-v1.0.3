...
export default function AttendanceManagement() {
  ...
  // ========== FUNGSI PEMBANTU UNTUK MEMBUAT TIMESTAMP DARI TANGGAL TERTENTU ==========
  const getTimestampForDate = (tanggal: string): string => {
    const now = new Date();
  ...
  const getTimestampForSelectedDate = (): string => {
    return getTimestampForDate(selectedTanggal);
  };

  // ========== RESET AUTO-ALFA SAAT TANGGAL ATAU KELAS BERUBAH ==========
  useEffect(() => {
    setAutoAlfaProcessedHarian(false);
  }, [selectedTanggal, selectedKelasHarian]);
  ...
  useEffect(() => {
    if (selectedKelasHarian && kelasListHarian.length > 0) {
      const isValid = kelasListHarian.some(
        (k) => k.id_kelas.toString() === selectedKelasHarian
      );
  ...
  useEffect(() => {
    if (selectedKelasMapel && kelasListMapel.length > 0) {
      const isValid = kelasListMapel.some(
        (k) => k.id_kelas.toString() === selectedKelasMapel
      );
  ...
  const uniqueDays = Array.from(new Set(filteredJadwalList.map((j) => j.hari))).sort(
    (a, b) => {
  ...
  const jadwalByDay = selectedDay
    ? filteredJadwalList.filter((j) => j.hari === selectedDay)
    : [];
  ...
  const fetchPresensiHarian = async (
    ...
    // Cek apakah tanggal adalah Sabtu atau Minggu
    const dateObj = new Date(tanggal + "T00:00:00");
    const day = dateObj.getDay();
    if (day === 0 || day === 6) {
      toast({
        title: "Hari Libur",
        description: "Presensi tidak dapat dilakukan pada hari Sabtu dan Minggu",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // Auto Alfa dengan batch insert
      if (
        presensiTypeHarian === "masuk" &&
        !skipAutoAlfa &&
        !autoAlfaProcessedHarian &&
        combined.some((p) => !p.status_presensi)
      ) {
        console.log("🚀 Auto-Alfa triggered for date:", tanggal);
        setAutoAlfaProcessedHarian(true);
        const belumAbsen = combined.filter((p) => !p.status_presensi);

        const insertData = belumAbsen
          .filter((item) => !item.id_pres_harian)
          .map((item) => ({
            id_siswa: item.id_siswa,
            status_presensi: "Alfa",
            waktu_presensi: getTimestampForDate(tanggal),
          }));

        if (insertData.length > 0) {
          try {
            console.log(`Inserting ${insertData.length} records with tanggal ${tanggal}...`);
            const { data, error } = await supabase
              .from("presensi_harian")
              .insert(insertData)
              .select();
            if (error) {
              console.error("❌ Error inserting batch Alfa:", error);
              toast({
                title: "Error",
                description: `Gagal mengisi Alfa: ${error.message}`,
                variant: "destructive",
              });
            } else {
              console.log("✅ Insert success, inserted rows:", data);
              toast({
                title: "Info",
                description: `Siswa yang belum absen masuk otomatis diisi Alfa (${data.length} siswa) untuk tanggal ${tanggal}`,
              });
            }
          } catch (err) {
            console.error("❌ Exception inserting batch Alfa:", err);
          }
        } else {
          console.log("No new records to insert (all already have presensi)");
        }
        // Refresh data tanpa auto-Alfa lagi
        await fetchPresensiHarian(kelasId, tanggal, true);
      }
    } catch (error: any) {
      console.error("Error pada fetchPresensiHarian:", error);
      toast({ title: "Error", description: error.message, variant: "destructive" });
    } finally {
  ...
  const savePresensiHarian = async () => {
    // Hanya admin/admin_jurusan yang boleh menyimpan untuk tanggal selain hari ini
    const today = new Date().toISOString().split("T")[0];
    if (
      user?.peran !== "admin" &&
      user?.peran !== "admin_jurusan" &&
      selectedTanggal !== today
    ) {
      toast({
        title: "Error",
        description: "Hanya admin yang dapat mengubah presensi untuk tanggal sebelumnya",
        variant: "destructive",
      });
      return;
    }
    ...
    if (presensiTypeHarian === "masuk") {
          ...
          if (existing?.id_pres_harian) {
            await supabase
              ...
              .update({
                ...
                waktu_presensi: getTimestampForSelectedDate(),
              })
      ...
      try {
        for (const item of presensiHarian) {
          const desiredPulang =
            pendingHarianPulangMap.get(item.id_siswa) ?? (item.status_pulang === "Pulang");
            ...
            if (item.id_pres_harian) {
              await supabase
                ...
                .update({
                  ...
                  waktu_pulang: getTimestampForSelectedDate(),
                })
  ...
  const handlePulangChange = (siswaId: number, isPulang: boolean) => {
    setPendingHarianPulangMap((prev) => {
      const newMap = new Map(prev);
      const original =
        presensiHarian.find((p) => p.id_siswa === siswaId)?.status_pulang === "Pulang";
  ...
  const fetchPresensiMapel = async (jadwalParam?: Jadwal | null, skipAutoAlfa = false) => {
    ...
    try {
      ...
      if (
        !skipAutoAlfa &&
        !autoAlfaProcessedMapel &&
        combined.some((p) => !p.status)
      ) {
        ...
        toast({
          title: "Info",
          description: "Siswa yang tidak scan QR otomatis diisi Alfa",
        });
      }
    } catch (error: any) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
  ...
  const generateQRCode = async (jadwal: Jadwal) => {
    ...
    if (currentDay !== expectedDay) {
      toast({
        ...
        description: `QR Code hanya dapat digenerate pada hari ${jadwal.hari} (hari ini ${now.toLocaleDateString(
          "id-ID",
          { weekday: "long" }
        )})`,
        variant: "destructive",
  ...
  const isAdmin = user?.peran === "admin" || user?.peran === "admin_jurusan";
      ...
      <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-8 space-y-6 sm:space-y-8">
            ...
            >
              <div className="flex justify-center">
                <TabsList className="bg-[#2C5EAD] p-1 rounded-xl">
                  <TabsTrigger
                    value="harian"
                    className="rounded-lg data-[state=active]:bg-white data-[state=active]:text-[#2C5EAD] data-[state=active]:shadow-sm px-3 sm:px-4 py-1.5 text-xs sm:text-sm gap-2 text-white/80"
                  >
                ...
                {user?.peran === "guru" && kelasListHarian.length === 0 ? (
                  <Alert className="rounded-lg bg-amber-50 border-amber-200">
                    ...
                    <AlertDescription className="text-amber-700 text-xs sm:text-sm">
                      Anda tidak memiliki akses ke presensi harian karena hanya wali kelas
                      yang dapat mengelola presensi harian.
                    </AlertDescription>
                  </Alert>
                ) : (
                  <>
                    <div className="flex flex-col gap-3">
                      <div className="flex flex-wrap gap-3 items-end">
                        <div className="w-full sm:w-56">
                          <Label className="text-slate-700 text-xs sm:text-sm font-medium">
                            Kelas
                          </Label>
                          <Popover
                            open={popoverHarianOpen}
                            onOpenChange={setPopoverHarianOpen}
                          >
                              ...
                              <div className="p-2 border-b bg-slate-50">
                                <div className="flex gap-1 mb-2 flex-wrap">
                                  {["all", "X", "XI", "XII"].map((jenjang) => (
                                    <Button
                                      key={jenjang}
                                      variant={
                                        kelasHarianJenjangFilter === jenjang
                                          ? "default"
                                          : "ghost"
                                      }
                                      size="sm"
                                      ...
                                      onClick={() =>
                                        setKelasHarianJenjangFilter(jenjang)
                                      }
                                    >
                                ...
                                <div className="relative">
                                  ...
                                  <Input
                                    ...
                                    value={kelasHarianSearchQuery}
                                    onChange={(e) =>
                                      setKelasHarianSearchQuery(e.target.value)
                                    }
                                    className="pl-7 h-8 text-sm rounded-lg"
                                    onClick={(e) => e.stopPropagation()}
                                  />
                                  {kelasHarianSearchQuery && (
                                    <button
                                      onClick={() => setKelasHarianSearchQuery("")}
                                  ...
                                  filteredKelasHarianOptions.map((kelas) => (
                                    <button
                                      ...
                                      className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 transition-colors ${
                                        selectedKelasHarian ===
                                        kelas.id_kelas.toString()
                                          ? "bg-[#C4E2F5] text-[#2C5EAD] font-medium"
                                          : "text-slate-700"
                                      }`}
                                      onClick={() => {
                                        handleSelectKelasHarian(
                                          kelas.id_kelas.toString()
                                        );
                        ...
                        <div className="flex-1 sm:w-40">
                          ...
                          <Input
                            ...
                            className="rounded-lg border-slate-200 h-8 sm:h-9 text-xs sm:text-sm w-full"
                            max={
                              isAdmin
                                ? undefined
                                : new Date().toISOString().split("T")[0]
                        ...
                        <Button
                          ...
                          disabled={!selectedKelasHarian || isFetchingHarian}
                          className="rounded-lg h-8 sm:h-9 px-3 text-xs sm:text-sm shrink-0 border-[#2C5EAD] text-[#2C5EAD] hover:bg-[#2C5EAD] hover:text-white"
                        >
                          <RefreshCw
                            className={`mr-1.5 h-3.5 w-3.5 ${
                              isFetchingHarian ? "animate-spin" : ""
                            }`}
                          />{" "}
                          Refresh
                        </Button>
                      ...
                      <div className="flex justify-center mt-2">
                        <div className="inline-flex bg-white rounded-full p-1 shadow-md border border-slate-200 gap-1">
                          ...
                          >
                            <LogIn
                              className={`h-4 w-4 transition-transform ${
                                presensiTypeHarian === "masuk" ? "animate-pulse" : ""
                              }`}
                            />
                          ...
                          >
                            <LogOut
                              className={`h-4 w-4 transition-transform ${
                                presensiTypeHarian === "pulang" ? "animate-pulse" : ""
                              }`}
                            />
                    ...
                    {selectedKelasHarian && (
                      <div className="border rounded-lg overflow-hidden shadow-sm">
                          ...
                          <Table className="w-full">
                            <TableHeader>
                              <TableRow className="bg-slate-50">
                                <TableHead className="font-semibold text-center text-xs sm:text-sm w-24">
                                  NIS
                                </TableHead>
                                <TableHead className="font-semibold text-center text-xs sm:text-sm min-w-[140px]">
                                  Nama Siswa
                                </TableHead>
                                <TableHead className="font-semibold text-center text-xs sm:text-sm w-28">
                                  Status
                                </TableHead>
                                {presensiTypeHarian === "masuk" ? (
                                  STATUS_HARIAN_SEKOLAH.map((status) => (
                                    <TableHead
                                      key={status}
                                      className="text-center font-semibold text-xs sm:text-sm min-w-[80px]"
                                    >
                                      <div className="flex flex-col items-center gap-1">
                                        <span>{status}</span>
                                        <Checkbox
                                          checked={pendingBulkStatus === status}
                                          onCheckedChange={() =>
                                            handleBulkStatusMasuk(status)
                                          }
                                    ...
                                    <div className="flex flex-col items-center gap-1">
                                      ...
                                      <Checkbox
                                        checked={
                                          ...
                                          presensiHarian.every(
                                            (item) =>
                                              (pendingHarianPulangMap.get(
                                                item.id_siswa
                                              ) ??
                                                (item.status_pulang === "Pulang")) ===
                                              true
                                          )
                                        }
                            ...
                            <TableBody>
                              {isFetchingHarian ? (
                                <TableRow>
                                  <TableCell
                                    colSpan={
                                      3 +
                                      (presensiTypeHarian === "masuk"
                                        ? STATUS_HARIAN_SEKOLAH.length
                                        : 1)
                                    }
                                    className="text-center py-10"
                                  >
                                    <Loader2 className="h-6 w-6 animate-spin mx-auto text-[#2C5EAD]" />
                                  </TableCell>
                                </TableRow>
                              ) : presensiHarian.length === 0 ? (
                                <TableRow>
                                  <TableCell
                                    ...
                                    className="text-center py-10 text-slate-500 text-xs sm:text-sm"
                                  >
                                ...
                                presensiHarian.map((item) => {
                                  const isPKL = item.siswa?.id_pkl !== null;
                                  if (presensiTypeHarian === "masuk") {
                                    const availableStatus = isPKL
                                      ? STATUS_HARIAN_PKL
                                      : STATUS_HARIAN_SEKOLAH;
                                    const currentStatus =
                                      pendingHarianMasuk.get(item.id_siswa) ??
                                      ...
                                      >
                                        ...
                                        <TableCell className="text-center">
                                          {isPKL ? (
                                            ...
                                            <span className="px-2 py-0.5 rounded-full text-[10px] sm:text-xs bg-green-100 text-green-700">
                                              Sekolah
                                            </span>
                                          )}
                                        </TableCell>
                                        {STATUS_HARIAN_SEKOLAH.map((status) => {
                                          if (!availableStatus.includes(status))
                                            return (
                                              <TableCell
                                                key={status}
                                                className="text-center bg-slate-50/30"
                                              ></TableCell>
                                            );
                                          return (
                                            <TableCell
                                    ...
                                    const isPulang =
                                      pendingHarianPulangMap.get(item.id_siswa) ??
                                      (item.status_pulang === "Pulang");
                                    return (
                                      <TableRow
                                        key={item.id_siswa}
                        ...
                        <div className="flex justify-end gap-2 p-3 bg-slate-50 border-t">
                          <Button
                            variant="outline"
                            onClick={resetPendingHarian}
                            disabled={
                              isSavingHarian ||
                              (presensiTypeHarian === "masuk"
                                ? pendingHarianMasuk.size === 0
                                : pendingHarianPulangMap.size === 0)
                            }
                            className="rounded-lg h-8 px-3 text-xs border-slate-300"
                          >
                            <RotateCcw className="mr-1.5 h-3.5 w-3.5" /> Reset
                          </Button>
                          <Button
                            onClick={savePresensiHarian}
                            disabled={
                              isSavingHarian ||
                              (presensiTypeHarian === "masuk"
                                ? pendingHarianMasuk.size === 0
                            ...
                            className="rounded-lg h-8 px-3 text-xs bg-[#2C5EAD] hover:bg-[#1e4a8a] text-white"
                          >
                            {isSavingHarian ? (
              ...
              <TabsContent value="mapel" className="space-y-4 sm:space-y-5">
                <div className="flex flex-col sm:flex-row gap-4 items-start">
                  <div className="w-full sm:w-64 flex-shrink-0">
                    ...
                    <Popover
                      open={popoverMapelOpen}
                      onOpenChange={setPopoverMapelOpen}
                    >
                      <PopoverTrigger asChild>
                        <Button
                          variant="outline"
                      ...
                      <PopoverContent className="w-64 p-0" align="start" sideOffset={5}>
                        <div className="p-2 border-b bg-slate-50">
                          <div className="flex gap-1 mb-2 flex-wrap">
                            {["all", "X", "XI", "XII"].map((jenjang) => (
                              <Button
                                key={jenjang}
                                variant={
                                  kelasMapelJenjangFilter === jenjang
                                    ? "default"
                                    : "ghost"
                                }
                                size="sm"
                                ...
                                onClick={() => setKelasMapelJenjangFilter(jenjang)}
                              >
                                {jenjang === "all" ? "Semua" : jenjang}
                              </Button>
                            ))}
                          ...
                          <div className="relative">
                            ...
                            <Input
                              placeholder="Cari kelas..."
                              value={kelasMapelSearchQuery}
                              onChange={(e) => setKelasMapelSearchQuery(e.target.value)}
                              className="pl-7 h-8 text-sm rounded-lg"
                              onClick={(e) => e.stopPropagation()}
                            />
                            {kelasMapelSearchQuery && (
                              <button
                                onClick={() => setKelasMapelSearchQuery("")}
                                className="absolute right-2 top-1/2"
                              >
                                <X className="h-3.5 w-3.5 text-slate-400" />
                              </button>
                          ...
                          {filteredKelasMapelOptions.length === 0 ? (
                            ...
                            filteredKelasMapelOptions.map((kelas) => (
                              <button
                                key={kelas.id_kelas}
                                className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 transition-colors ${
                                  selectedKelasMapel === kelas.id_kelas.toString()
                                    ? "bg-[#C4E2F5] text-[#2C5EAD] font-medium"
                                    : "text-slate-700"
                                }`}
                                onClick={() => {
                                  setSelectedKelasMapel(kelas.id_kelas.toString());
                                  setPopoverMapelOpen(false);
                                  setKelasMapelSearchQuery("");
                                  setKelasMapelJenjangFilter("all");
                                }}
                              >
                                {kelas.nama}
                              </button>
                  ...
                  <div className="flex-1 w-full">
                    ...
                    {selectedKelasMapel && filteredJadwalList.length === 0 && (
                      <Alert className="rounded-lg bg-amber-50 border-amber-200">
                        ...
                        <AlertDescription className="text-amber-700 text-xs sm:text-sm">
                          Tidak ada jadwal yang dapat diakses untuk kelas ini
                        </AlertDescription>
                      </Alert>
                    )}
                    {selectedKelasMapel && filteredJadwalList.length > 0 && (
                      ...
                      {jadwalByDay.map((jadwal) => {
                        ...
                        return (
                          <Card
                            ...
                            className={`group relative overflow-hidden transition-all duration-200 cursor-pointer border-0 shadow-md hover:shadow-lg hover:-translate-y-1 ${
                              isSelected
                                ? "ring-2 ring-[#2C5EAD]"
                          ...
                          >
                            ...
                            <CardContent className="p-4 pt-3 space-y-3 bg-white rounded-b-xl">
                              ...
                              <div className="flex justify-end pt-2">
                                ...
                                >
                                  <QrCode className="h-3.5 w-3.5 mr-1.5" /> Generate QR
                                </Button>
                              </div>
                              {isSelected && (
                  ...
                  <div className="mt-8 border-t pt-5">
                    <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                      <div>
                        <h3 className="text-base sm:text-lg font-semibold text-slate-800 flex items-center gap-2">
                          <BookOpen className="h-5 w-5 text-[#2C5EAD]" /> Presensi{" "}
                    ...
                    <div className="border rounded-lg overflow-hidden shadow-sm">
                              ...
                              {STATUS_MAPEL.map((status) => (
                                ...
                                >
                                  <div className="flex flex-col items-center gap-1">
                                    <span>{status}</span>
                                    <Checkbox
                                      checked={pendingBulkStatusMapel === status}
                                      onCheckedChange={() =>
                                        handleBulkStatusMapel(status)
                                      }
                          ...
                          <TableBody>
                            {isFetchingMapel ? (
                              ...
                              presensiMapel.map((item) => {
                                const currentStatus =
                                  pendingMapel.get(item.id_siswa) ?? item.status ?? "";
                                return (
                                  <TableRow
                                    key={item.id_siswa}
                                    className="hover:bg-slate-50 transition-colors"
                                      ...
                                      >
                                        <div className="flex justify-center items-center">
                                          <RadioGroup
                                            value={currentStatus}
                                            onValueChange={(val) =>
                                              handleMapelChange(item.id_siswa, val)
                                            }
                                            disabled={isSavingMapel}
                                            className="flex justify-center"
                                          >
                      ...
                      <div className="flex justify-end gap-2 p-3 bg-slate-50 border-t">
                        <Button
                          variant="outline"
                          onClick={resetPendingMapel}
                          disabled={isSavingMapel || pendingMapel.size === 0}
                          className="rounded-lg h-8 px-3 text-xs border-slate-300"
                        >
                          <RotateCcw className="mr-1.5 h-3.5 w-3.5" /> Reset
                        </Button>
                        <Button
                          onClick={savePresensiMapel}
                          disabled={isSavingMapel || pendingMapel.size === 0}
                          className="rounded-lg h-8 px-3 text-xs bg-[#2C5EAD] hover:bg-[#1e4a8a] text-white"
                        >
                          {isSavingMapel ? (
          ...
          <DialogContent className="sm:max-w-md max-w-[95vw] rounded-xl p-4 sm:p-6">
            <DialogHeader>
              <DialogTitle className="text-base sm:text-lg flex items-center gap-2">
                <QrCode className="h-5 w-5 text-[#2C5EAD]" /> QR Code Presensi (Dinamis 30
                detik)
              </DialogTitle>
            </DialogHeader>
            <div className="flex flex-col items-center space-y-3 py-3">
              ...
              <div className="text-center space-y-0.5">
                ...
                <p className="text-[10px] text-amber-600 mt-1">
                  ⚠️ QR Code berubah setiap 30 detik dan hanya berlaku 30 detik. Tidak
                  bisa dipakai ulang.
                </p>
              </div>
            </div>
            <DialogFooter>
              <Button
                ...
                className="rounded-lg h-8 sm:h-9 text-xs sm:text-sm border-[#2C5EAD] text-[#2C5EAD] hover:bg-[#2C5EAD] hover:text-white"
              >
                Tutup
              </Button>
            </DialogFooter>
        ...
        <Dialog open={confirmHarianOpen} onOpenChange={setConfirmHarianOpen}>
          <DialogContent className="sm:max-w-md max-w-[95vw] rounded-xl p-4 sm:p-6">
            ...
            <div className="space-y-2 text-sm text-slate-600">
              ...
              <div className="bg-slate-50 p-3 rounded-lg">
                <p>
                  <strong>Kelas:</strong>{" "}
                  {kelasListHarian.find(
                    (k) => k.id_kelas.toString() === pendingHarianKelas
                  )?.nama || pendingHarianKelas}
                </p>
                <p>
                  <strong>Tanggal:</strong> {pendingHarianTanggal}
                </p>
                <p>
                  <strong>Jenis:</strong>{" "}
                  {presensiTypeHarian === "masuk" ? "Presensi Masuk" : "Presensi Pulang"}
                </p>
              </div>
              {presensiTypeHarian === "masuk" && (
                <p className="text-xs text-amber-600 mt-2">
                  ⚠️ Siswa yang belum melakukan presensi masuk akan otomatis diisi{" "}
                  <strong>Alfa</strong> (kecuali sudah ada presensi sebelumnya).
                </p>
              )}
            </div>
            <DialogFooter className="gap-2 sm:gap-0">
              <Button
                variant="outline"
                onClick={() => setConfirmHarianOpen(false)}
                className="rounded-lg"
              >
                Batal
              </Button>
              <Button
                onClick={confirmHarian}
                className="rounded-lg bg-[#2C5EAD] hover:bg-[#1e4a8a] text-white"
              >
        ...
        <Dialog open={confirmMapelOpen} onOpenChange={setConfirmMapelOpen}>
          <DialogContent className="sm:max-w-md max-w-[95vw] rounded-xl p-4 sm:p-6">
            ...
            <div className="space-y-2 text-sm text-slate-600">
              ...
              <div className="bg-slate-50 p-3 rounded-lg">
                <p>
                  <strong>Mata Pelajaran:</strong> {pendingMapelJadwal?.mata_pelajaran}
                </p>
                <p>
                  <strong>Kelas:</strong> {pendingMapelJadwal?.kelas_nama}
                </p>
                <p>
                  <strong>Hari & Jam:</strong> {pendingMapelJadwal?.hari},{" "}
                  {pendingMapelJadwal?.jam}
                </p>
                <p>
                  <strong>Guru:</strong> {pendingMapelJadwal?.guru}
                </p>
              </div>
              <p className="text-xs text-amber-600 mt-2">
                ⚠️ Siswa yang belum melakukan scan QR akan otomatis diisi{" "}
                <strong>Alfa</strong>.
              </p>
            </div>
            <DialogFooter className="gap-2 sm:gap-0">
              <Button
                variant="outline"
                onClick={() => setConfirmMapelOpen(false)}
                className="rounded-lg"
              >
                Batal
              </Button>
              <Button
                onClick={confirmMapel}
                className="rounded-lg bg-[#2C5EAD] hover:bg-[#1e4a8a] text-white"
              >
                Ya, Buka Presensi
              </Button>
        ...
        <div className="text-center pt-3">
          ...
          <p className="text-xs text-slate-400">
            © {new Date().getFullYear()} Manajemen Presensi - TITEN
          </p>
